...
The script defines an ordered dict `arrprocessscope = {intindex: strdesc, …}` near the top of `tmdb-movie-preprocess.py` and runs `for intindex, strdesc in arrprocessscope.items():` — **process selection is just this dict, and execution order is dict insertion order, not numeric order.** Many candidate scopes are present as commented-out lines; to run a subset, comment the full default and uncomment (or add) a narrower dict. There is **no internal scheduler loop** in the active script — it runs the scope once and exits; recurrence is handled externally (Docker run + host cron / launcher). The archived Process 30 (`archive/TMDB_MOVIE_LANG_META.py`) is not in the default scope, and its spaCy-based lemmatisation has been removed from the codebase.

Default scope (insertion order): `0` (HTML-unescape `T_WC_CUSTOM_LIST`), `1` (`WIKIPEDIA_FORMAT_LINE` + technical-junction enrichment), `2` (`T2S_MOVIE_TECHNICAL`), `62` (link Wikidata → T2S technical), `60` (link Wikidata → topics), `3` (`T2S_TOPIC`), `41` (`T2S_COLLECTION`), `61` (link Wikidata → collections), `42` (`T2S_LIST`), `43` (`T2S_GROUP`), `44` (`T2S_AWARD`), `47` (`T2S_NOMINATION`), `45` (`T2S_MOVEMENT`), `46` (`T2S_DEATH`), then the bulk copy/rebuild steps `4`–`40` (`T2S_MOVIE`, `T2S_SERIE`, `T2S_PERSON`, `T2S_COMPANY`, `T2S_NETWORK`, the person/genre/company/network/country/language link tables, image & video tables, seasons/episodes and their links/media, and finally `T2S_ITEM`).
