...
- **Enrichment** (`IMDB_RATING`, `IMDB_RATING_WEIGHTED` from `T_WC_IMDB_MOVIE_RATING_IMPORT` via `ID_IMDB` join, for seasons that carry an IMDb id): full-table set-based UPDATEs run **once per run** (previously per chunk), with the global IMDb weighted-rating average computed once up-front instead of via a per-chunk `CROSS JOIN`. In practice almost no season carries an IMDb id, because **IMDb rates titles and episodes but never seasons**, so these two columns are filled for real by the **season rollup at the end of Process 28** (see below), which derives them from the season's episodes. That rollup runs in 28 and not here precisely because 27 executes *before* 28: computing it here would average the previous run's episode ratings and stay one run behind forever.
...
- Enriches `IMDB_RATING`, `IMDB_VOTES` and `IMDB_RATING_WEIGHTED` from `T_WC_IMDB_MOVIE_RATING_IMPORT` (most episodes lack an `ID_IMDB`, so this populates a sparse minority: measured coverage 2026-07-26 is ~6 % of episodes aired in 2024, ~5 % in 2025, ~4 % in 2026, the denominator being saturated with daily soaps and local productions that carry no IMDb id) — set-based UPDATEs covering the **whole table once per run**, because the IMDb source changes independently of an episode's `TIM_UPDATED`, with the global IMDb weighted-rating average computed once up-front instead of via a per-chunk `CROSS JOIN`. Since **TMDB-MOVIE-PREPROCESS-033** these passes are **chunked by `ID_EPISODE` range** (5000, same granularity as the base copy): coverage is identical, only the transaction size changes, so a table in the millions of rows no longer holds one long write lock. `IMDB_VOTES` is carried next to the rating because a rating alone is not interpretable on an episode (9.3 on 43 000 votes and 9.3 on 12 votes are different claims) and because consumers need it to prefer the IMDb figure over the far thinner TMDb `VOTE_COUNT`. The column is added idempotently (`ADD COLUMN IF NOT EXISTS`) at the start of the process, so the step self-installs and stays safe to re-run.
...
- **Season rollup** (**TMDB-MOVIE-PREPROCESS-034**), the last step of the process, writing to `T_WC_T2S_SEASON`: derives each season's IMDb rating from its own episodes, since IMDb never rates seasons. `IMDB_RATING` is the **plain mean** of the season's rated episodes, deliberately not vote-weighted (opening episodes are always the most watched, so vote-weighting would systematically over-represent the start of a season). `IMDB_RATING_WEIGHTED` applies the **same bayesian shrinkage** as movies, series and episodes to that mean, using the season's summed episode votes as the vote mass, so the column keeps one meaning across every entity type and cross-type sorts stay valid. Episodes with no rating are excluded rather than counted as zero, so a season still airing is scored on the episodes actually aired and rated; both aggregates use the same episode population. Chunked by `ID_SEASON` range (5000). Worked example, House of the Dragon season 3 on 2026-07-27 with five of eight episodes rated: mean **8.42** over 158 010 votes, shrunk to roughly **8.24**. Known edge, not handled: a season whose episodes all lose their ratings keeps its last computed value, since the pass is an `INNER JOIN` and never resets.
